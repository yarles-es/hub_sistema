...
export class UsuarioController {
  ...
  async create(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const usuario = await this.createUsuarioService.execute(req.body);
  ...
  async getByEmail(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const usuario = await this.findUsuarioByEmailService.execute(req.query.email as string);
      res.status(200).json(usuario);
    } catch (error) {
      next(error);
    }
  ...
  async update(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const updatedUsuario = await this.updateUsuarioService.execute(Number(req.params.id), req.body);
