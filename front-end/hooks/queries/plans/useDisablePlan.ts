...
import { disablePlan } from "@/api/plano/plano.api";
import {
  DataMutation,
  ErrMutation,
...
export function useDisablePlan(options?: OptionsTypeUseMutation<Variables>) {
  const qc = useQueryClient();

  return useMutation<DataMutation, ErrMutation, Variables>({
    mutationFn: (payload) => disablePlan(payload),
